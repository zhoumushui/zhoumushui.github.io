...
$(window).load(function () {
    ...
    if (!$('.login, .four-zero')[0] && getHtmlName() == 'index') { // AZ:只在主页显示
        notify('时间过得好快！', 'info'); // 'info','-light'
    }
});
...
$(document).ready(function () {
    ...
    /*--------------------------------------
     ...
     ---------------------------------------*/
    function notify(from, align, icon, type, animIn, animOut, toastContent) {
        $.notify({
            icon: icon,
            title: '',
    ...
    $('.notifications > div > .btn').click(function (e) {
        ...
        var toastContent = $(this).attr('data-toast-content'); // AZ: toast content

        notify(nFrom, nAlign, nIcons, nType, nAnimIn, nAnimOut, toastContent);
    });
