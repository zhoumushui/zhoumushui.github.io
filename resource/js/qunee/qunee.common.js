    ...
    JSONSerializer.prototype = {
        ...
        toJSON: function (value) {
            ...
            if (value._refId === undefined) {
                ...
                //æ·»åŠ å¼•ç”¨æ ‡è®°ï¼Œä¸‹æ¬¡é‡åˆ°è¿™ä¸ªå¯¹è±¡æ—¶ï¼Œä¸éœ€è¦å†toJSONï¼Œè€Œæ˜¯ç›´æŽ¥è¾“å‡ºå¼•ç”¨ï¼Œæ¯”å¦‚{"$ref": 1}
                var id = value._refId = this._index++;
                //å°†å¯¹è±¡æš‚æ—¶å­˜æ”¾åœ¨_refValuesä¸­ï¼Œä»¥ä¾¿åœ¨å¯¼å‡ºå®ŒæˆåŽï¼Œåˆ é™¤æŽ‰ä¸Šä¸€æ­¥å¯¹valueå¢žåŠ çš„_refIdå±žæ€§
                this._refValues[id] = value;
                this._refs[id] = json;
                return json;
            }
            //é‡åˆ°ç›¸åŒçš„å¯¹è±¡ï¼Œå°†å¯¹è±¡ä¿¡æ¯å­˜æ”¾åˆ°å…¨å±€mapï¼Œç½‘å…ƒå±žæ€§åªéœ€è¦å­˜æ”¾å¼•ç”¨idï¼Œæ¯”å¦‚{"$ref": 1}
            //å…¨å±€mapä¸­å­˜æ”¾åœ¨gå±žæ€§ä¸­ï¼Œä»¥idä¸ºkeyï¼Œjsonä¸ºvalueï¼Œå¦‚ä¸‹ï¼š
            //"refs": {
            //  "1": {
            ...
            if (!this._globalRefs[id]) {
                //å¦‚æžœè¿˜æ²¡æœ‰åŠ å…¥åˆ°å…¨å±€å¼•ç”¨åŒºï¼Œåˆ™å°†jsonæ”¾å…¥åˆ°_globalRefsï¼ŒåŒæ—¶å°†åŽŸæ¥çš„jsonå˜æˆå¼•ç”¨æ–¹å¼
        ...
        jsonToElement: function (json) {
            //å¦‚æžœä¹‹å‰è§£æžçš„æ•°æ®ä¸­å¼•ç”¨åˆ°äº†æ­¤èŠ‚ç‚¹ï¼Œæ­¤èŠ‚ç‚¹å…¶å®žå·²ç»è¢«è§£æžäº†ï¼Œè¿™é‡Œåªéœ€è¦è¿”å›žå¼•ç”¨å°±å¯ä»¥äº†
        ...
        parseJSON: function (json) {
            ...
            if (json.$ref !== undefined) {
                //ä»Žå…¨å±€å¼•ç”¨ä¸­èŽ·å–jsonä¿¡æ¯
                var gJson = this._globalRefs[json.$ref];
                if (!gJson) {
                    return;
                }
                //å°†jsonä¿¡æ¯è§£æžæˆå¯¹è±¡ï¼Œå¹¶ç¼“å­˜åœ¨jsonçš„_valueå±žæ€§ä¸­
                if (gJson._value === undefined) {
                    gJson._value = this.parseJSON(gJson);
                }
                return gJson._value;
            }
            //å¦‚æžœå±žæ€§ä¸ºelementå¼•ç”¨ï¼Œå…ˆä»Ž_elementRefsä¸­æ‰¾åˆ°å¯¹åº”elementçš„jsonä¿¡æ¯ï¼Œç„¶åŽå°†æ­¤jsonä¿¡æ¯è§£æžæˆelement
            ...
            ////å¦‚æžœjsonåŒ…å«_refIdå±žæ€§ï¼Œè¯´æ˜Žè¿™æ˜¯ä¸€ä¸ªelementç±»åž‹ï¼Œç›´æŽ¥è°ƒç”¨jsonToElementï¼Œä¸è¿‡åº”è¯¥ä¸ä¼šå‡ºçŽ°
...
//            dom.ontouchstart = onclick;
        ...
        getMenuItems: function (graph, data, evt) {
            ...
            /*
             items.push({
             text: getI18NString('清除'), action: function () {
             graph.clear();
             }
             })
             items.push(Q.PopupMenu.Separator);
            ...
            items.push({
                text: getI18NString('放大'), action: function (evt, item) {
                    var localXY = graph.globalToLocal(evt);
                    graph.zoomIn(localXY.x, localXY.y, true);
                }
            });
            items.push({
                text: getI18NString('缩小'), action: function (evt, item) {
                    var localXY = graph.globalToLocal(evt);
            ...
            items.push(Q.PopupMenu.Separator);
            items.push({html: '<a href="http://www.zhoumushui.com" target="_blank">zhoumushui' + '</a>'});
            */
            return items;
        }
    }
