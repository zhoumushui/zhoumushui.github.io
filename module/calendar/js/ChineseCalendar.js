var ChineseCalendar = {
    ...
    isBirthday: function (y, m, d, ly, lm, ld) {
        var lstr = lm > 9 ? (lm + '') : '0' + lm;
        lstr += ld > 9 ? (ld + '') : '0' + ld;
        var cstr = m > 9 ? (m + '') : '0' + m;
        cstr += d > 9 ? (d + '') : '0' + d;
        ...
        // 农历生日
        var festive = ChineseCalendar.lBirthday;
        for (var i = 0, l = festive.length; i < l; i += 1) {
            var test = festive[i].match(/(\d*)-(.*)/);
            if (test[1] === lstr) {
                return test[2];
        ...
        // 阳历生日
        festive = ChineseCalendar.cBirthday;
        for (var i = 0, l = festive.length; i < l; i += 1) {
            var test = festive[i].match(/(\d*#?\d*)-(.*)/);
            var year = test[1].split('#');
            if (year[1] && y >= year[1]) {
    ...
    lunarBirthday: function (date) {
        var result = ChineseCalendar.getlunarYMD(date);
        var isBirthday = ChineseCalendar.isBirthday(result.year, result.month, result.day, result.lunarYear, result.lunarMonth, result.lunarDay);
    ...
    date2lunar: function (date) {
        ...
        result.isToday = ChineseCalendar.isToday(date);
        //获取干支月
